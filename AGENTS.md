...
- After a passing gate, do NOT emit a second standalone "awaiting confirmation" line when the user already explicitly requested that phase.
- Reuse prior evidence by reference instead of restating full AC lists, task descriptions, risks, or earlier findings.
- `/plan` should output the gate block plus the plan itself — no extra transition chatter.
- `/review` should lead with the burden-of-proof table / findings and only mention changed evidence since `/implement`.
- `/test` should report commands, pass/fail counts, coverage deltas, and unresolved risks, not reprint the full test blueprint unless it changed.
- `/ship` should summarize final deltas, evidence references, and remaining constraints without replaying the full implementation/review/test narrative.
- Prefer a short pointer such as `Ref: Work Log Evidence / prior AC map` over re-narrating the same material.
