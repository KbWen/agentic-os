...
7. **Skill Notes Cache Contract**: `## Skill Notes` is the approved cache for loaded skills. Leave as `none` until a phase loads a note. Each skill block MUST use: `### <skill-id>`, optional `- cached_hash: <sha256[:8]>`, `- First Loaded Phase: <phase>`, `- Applies To: <phases>`, one `#### <phase>` block per completed phase (≥2 Checklist bullets + 1 Constraint). Compaction MUST preserve `## Skill Notes` verbatim. Thresholds: `.agent/config.yaml §skill_cache_policy`.
...
- **Cache Check**: Prefer `## Skill Notes` cache when valid. Cache hit = matching `cached_hash` + valid phase block (≥2 Checklist + 1 Constraint + >50 chars body). Without hash, apply legacy validity check (missing block/phase, low bullet count, short body). Thresholds: `.agent/config.yaml §skill_cache_policy`.
...
3. **Evidence**: Compile reproducible evidence (specific commands, outputs, versions). "It should work" is NOT evidence.
