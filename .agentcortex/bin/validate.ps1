...
# Source-repo detection: has canonical deploy but no manifest (manifest is generated by deploy to downstream)
$isSourceRepo = (Test-Path -Path $canonicalDeploySh -PathType Leaf) -and -not (Test-Path -Path (Join-NormalPath $root '.agentcortex-manifest') -PathType Leaf)
...
if ($isSourceRepo) {
    $requiredDirsFiltered = @($requiredDirs | Where-Object { $_ -ne $claudeCommandsDir })
    Test-DirGroup -Label 'required framework directories present' -Paths $requiredDirsFiltered
} else {
