...
### D-10: External review adjudication (8 findings, 7 adopted, 1 partially overruled)
...
- **Final validators** (terminal write; both runs postdate every state write, including the external-review adoptions — the fifth pass of the contract's own "fix and re-run until a run and its recording land clean" loop, every iteration driven by a real finding rather than churn): `validate.sh` **`pass=118 warn=4 fail=0 skip=2`** and `validate.ps1` **`pass=118 warn=4 fail=0 skip=2`** — exact parity, `fail=0`.
