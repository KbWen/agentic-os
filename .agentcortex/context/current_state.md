...
- **Last Updated**: 2026-06-09T00:20:00+08:00
...
### Ship-claude-adoring-cerf-IlMYh-worklog-lock-test-timebomb-2026-06-08
- **Branch `claude/adoring-cerf-IlMYh`** (quick-win, test fix) — Fixed a permanently-failing frozen-NOW time-bomb in `tests/guard/test_worklog_lock_recovery.py::test_active_lock_preserved_by_api_and_cli`. The test's CLI-subprocess half classifies a lock against the **real wall clock** but the lock was stamped with the frozen module-level `NOW` (2026-06-08T12:15Z); once wall-clock passes `NOW`+`stale_timeout_minutes` (60) the CLI recovers the lock (rc 0, `stale-time`) instead of preserving it (rc 2), so the assertion fails on every run after 13:15Z — already detonated on main.
  - **Investigation outcome**: the underlying `recover_worklog_lock.classify_lock` time-overrides-live-PID behavior is **deliberate** (spec `worklog-lock-auto-recovery.md` Constraints: advisory + age-based; #188 recovers hung/abandoned holders). Default path writes **no PID**, so there is no live-PID being overridden in normal use; `updated_at` refreshes at phase boundaries (no heartbeat). No lock-semantics change warranted — only the test was defective. PR #206 (chat-language policy) was mislabeled as the fix; it is unrelated.
  - **Fix**: stamp the CLI-half lock `updated_at` from the real clock (`datetime.now(timezone.utc)`) and pass the same `now=` to the API call, mirroring the already-safe sibling CLI tests; added an explanatory comment. Test-only, 1 file, no runtime code.
  - **Evidence**: before=`FAIL 0 != 2`; after=lock suite 8 passed, guard suite 140 passed, `validate.sh` pass=104 warn=3 fail=0. Rollback = revert the one-file diff.
