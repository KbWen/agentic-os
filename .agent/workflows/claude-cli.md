---
name: claude-cli
description: "[OPTIONAL MODULE] Run a task via Claude CLI while enforcing AgentCortex governance rules automatically."
tasks:
  - claude-cli
---
...
Dispatch a task to Claude CLI while ensuring AgentCortex governance compliance.

> This workflow wraps `claude` CLI calls with automatic Work Log creation, classification, prompt construction, and evidence collection. GPT-5.4 remains the orchestrator and final reviewer; Claude handles delegated implementation or testing subtasks.
...
| `feature` | delegated subtask only | `--model sonnet --permission-mode acceptEdits` | Keep orchestration and final review in GPT-5.4 |
...
4. **Update Work Log** with:
   ...
   - Test results
   - Whether GPT-5.4 accepted or rejected the delegated output
...
AI (internal):
  ...
  Post-flight: git diff verified, pytest -q run by GPT-5.4, evidence appended.
...
| Claude output unclear | GPT-5.4 reviews the diff manually and applies standard review |
| Task too complex for delegation | Reject delegation and continue with native execution |

## 6. Guardrails Integration

- All AgentCortex rules in `engineering_guardrails.md` apply to Claude-generated code.
- Claude CLI is treated as a **Junior Tool** — its output ALWAYS gets AI review before being accepted.
- GPT-5.4 remains the governance and acceptance layer; Claude is the delegated execution layer.
