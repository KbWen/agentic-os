...
- **No-Python fallback** (all SSoT writes here incl. §8 heartbeat): direct write + Drift Log entry (AGENTS.md §Write Isolation).
   - **Spec Index Cap**: Before updating Spec Index, count existing entries. If count ≥ `document_lifecycle.spec_index_max_entries` (default: 30 from `.agent/config.yaml`), keep the newest 30 inline and move older `shipped` **index lines** into the `## Spec Index Archive` section at the bottom of `current_state.md` (reuse it; never create a second). Spec files stay in `docs/specs/` — moving a body makes the entry a phantom and FAILs the completeness check.
