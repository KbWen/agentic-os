...
- **ADR-003's tamper evidence reaches downstream (PR #412)**: `ship.md` instructs every downstream ship to *append* to `INDEX.jsonl` via a deployed tool while `check_audit_chain.py` was never in the whitelist — write-only tamper evidence. It now ships, in **both** `deploy.sh` sites per frozen-spec AC-S5. **Upgrade note**: an adopter carrying a pre-existing broken chain flips green→red, and `append_chain_entry.py migrate` does **not** clear it (migrating rewrites committed lines, so the append-only witness fails instead). Both states are red and there is no clean path back without rewriting published history. That is the property of tamper evidence, not a defect.
